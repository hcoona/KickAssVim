...
def install_pathogen
  ...
  sh "curl -LSsko #{dir}/pathogen.vim https://raw.github.com/tpope/vim-pathogen/master/autoload/pathogen.vim"
end

def install_sensible
  dir = File.expand_path "#{File.expand_path("../", __FILE__)}/config"
  FileUtils.mkdir_p(dir) unless File.exists?(dir)
  sh "curl -LSsko #{dir}/sensible.vim https://raw.github.com/tpope/vim-sensible/master/plugin/sensible.vim"
end
